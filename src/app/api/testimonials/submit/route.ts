...
export async function POST(request: NextRequest) {
  try {
    ...
    // Handle Supabase record creation or update
    ...
    const recordData = {
      ...
      submitted_date: new Date().toISOString().split('T')[0], // YYYY-MM-DD format
      email: shouldStoreEmailInAirtable ? (body.email?.trim().toLowerCase() || null) : null, // Only store email if not in leads
      arrangement: 0, // Default to 0 (not arranged/displayed yet) - admin can set order number to display
      existing_lead_user: isExistingWaitlistUser, // Track if user was already in leads
      updated_date: new Date().toISOString().split('T')[0], // Track when it was last updated
      title_role: body.title?.trim() || getRandomItem(FUNNY_TITLES), // Random funny title if not provided
