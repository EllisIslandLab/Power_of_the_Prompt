...
function ConnectProjectContent() {
  ...
  return (
    <div className="min-h-screen bg-[#050714] relative">
      <div className="max-w-4xl mx-auto px-4 py-12 relative z-10">
        {/* Back to Portal Link */}
        <Link
          href="/portal"
          className="inline-flex items-center gap-2 text-[#b1c6f9] hover:text-white mb-6 transition-colors font-bold text-sm uppercase tracking-wider"
        >
          ← Back to Portal
        </Link>
        ...
        {/* Progress bar */}
        <div className="mb-8">
          <div className="h-2 bg-[#080c25] rounded-full overflow-hidden border border-white/10">
            <div
              className="h-full bg-[#FFB800] transition-all duration-500 ease-out"
              style={{ width: `${getStepProgress()}%` }}
            />
          </div>
          <div className="mt-2 flex justify-between text-[10px] text-[#c4c7c8] uppercase tracking-widest font-bold">
            {STEP_ORDER.map((step, idx) => (
              <span key={step} className={idx <= STEP_ORDER.indexOf(currentStep) ? 'text-[#FFB800]' : ''}>
                {step.replace('_', ' ')}
              </span>
            ))}
          </div>
        ...
        {/* Error display */}
        {error && (
          <div className="mb-6 p-4 bg-red-500/10 border border-red-500/30 rounded-lg text-red-300 flex items-start gap-3">
        ...
        {/* Welcome Step */}
        {currentStep === 'welcome' && (
          <div className="glass-panel rounded-2xl border-t-8 border-white p-8 md:p-12 text-center animate-fade-in relative">
            <div className="container-header">Initialize.v1</div>
            <h1 className="text-4xl font-bold text-white mb-4 mt-6">
              Connect Your Project
            </h1>
            <p className="text-base text-[#c4c7c8] mb-8 max-w-2xl mx-auto">
            ...
            <div className="grid md:grid-cols-3 gap-6 mb-8 text-left">
              <div className="p-4 bg-white/5 rounded-lg border border-white/10">
                <h3 className="font-semibold text-white mb-1 text-sm uppercase tracking-wider">GitHub Integration</h3>
                <p className="text-xs text-[#c4c7c8]">Secure access to your repository for creating branches and PRs</p>
              </div>
  ...
  return (
    <Suspense fallback={
      <div className="min-h-screen bg-[#050714] flex items-center justify-center">
        <div className="text-center">
          <div className="inline-block animate-spin rounded-full h-12 w-12 border-4 border-[#FFB800] border-t-transparent mb-4"></div>
          <p className="text-[#c4c7c8] text-sm uppercase tracking-wider font-bold">Loading...</p>
        </div>
      </div>
    }>
