/**
 * Script to add the Post-Video Session email template to the database
 *
 * Run with: node scripts/add-post-video-template.js
 */

const templateData = {
  name: "Post-Video Session Follow-Up",
  description: "Conclusory email sent to students after a 1-on-1 video consultation session",
  category: "follow-up",
  subjectTemplate: "Thanks for your time, {{name}}!",
  contentTemplate: `Thanks so much for your time, {{name}},
...
console.log('Template data prepared. Making API call to create template...')
...
fetch('http://localhost:3000/api/admin/templates', {
  ...
  .then(data => {
    if (data.success) {
      ...
      console.error('\n✗ Failed to create template:', data.error)
    }
  })
  .catch(error => {
    console.error('\n✗ Error making API call:', error.message)
    console.log('\nYou can also add this template manually through the admin UI at:')
    console.log('http://localhost:3000/admin/templates')
  })
